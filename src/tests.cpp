...
auto open_pcsx2() -> void {
    if (const char *env_p = std::getenv("PCSX2_TEST")) {
        ...
        snprintf(pcsx2_path, sizeof(pcsx2_path), "%s &", env_p);
...
#ifdef _WIN32
auto kill_pcsx2() -> int { return system("tskill pcsx2-qt"); }
#else
auto kill_pcsx2() -> int { return system("pkill pcsx2-qt"); }
        ...
        WHEN("We want to execute multiple operations in a row") {
            THEN("The operations get executed successfully and consistently") {
                ...
                REQUIRE_NOTHROW([&]() {
                    ...
                    REQUIRE(ipc.GetReply<PINE::PCSX2::MsgRead16>(resr, 2) == 7);
                    REQUIRE(ipc.GetReply<PINE::PCSX2::MsgRead32>(resr, 1) == 6);
                    REQUIRE(ipc.GetReply<PINE::PCSX2::MsgRead64>(resr, 0) == 5);
                }());
            }
